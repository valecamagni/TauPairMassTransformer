...
from utils_train import get_NFmodel
...
def test(model, test_ds, dataset, device, use_tauprod): 
    ...
    with torch.no_grad():
        for batch in test_ds:
            ...
            proj = model.flow_net(context_vector).rsample((1,))
            ...
            P_pt_1.append(torch.exp(proj[0, 0, 0]).item())
            P_pt_2.append(torch.exp(proj[0, 0, 1]).item())
            T_pt_1.append(torch.exp(target[0, 0]).item())
            T_pt_2.append(torch.exp(target[0, 1]).item())

            tpmt_mass = invariant_mass(torch.exp(proj[0, 0, 0]), taus[0, 0, 1], taus[0, 0, 2], torch.tensor(0.0), 
                                       torch.exp(proj[0, 0, 1]), taus[0, 1, 1], taus[0, 1, 2], torch.tensor(0.0))
